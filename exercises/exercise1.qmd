...
How close are the paramaeters to their simulated value?
```{r}
...
Now, fit in samEst - options for Stan:
...
#First we need to declare the model, here the 'static' Ricker (ac=FALSE indicates no autocorrelation, see further below):
m1<- samEst::compile_code(type='static', ac=FALSE, par='n',lambertW = FALSE)

#Then that model to our data:
m_stan <- samEst::ricker_stan(data=df,mod=m1) #Stan estimate
m_stan$mcmcsummary[1:4,] #summary for the Stan fit for our parameters of interest - look at Rhat as a quick diagnostic of the fit (should be <1.1)
...
```{r}
samEst::sr_plot(df=df,mod=m_stan$stanfit,type='static',form='stan',title='stan estimate')
...
In reality there is also error in our estimates of the adult recruits from a cohort (e.g. missing components of catch).
...
#Error in our estimate of recruits 
...
What if the sampling error changes over time - eg. change in methodology
...
What can you conclude about how sampling error influnces parameter estimates?
